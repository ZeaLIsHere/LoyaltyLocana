-- Reduce the per-customer stamp cooldown from 5 minutes to 10 seconds.
-- (Overrides the PRD default of 5 minutes at the user's request — the scanner
-- returns to idle after each scan, so a short cooldown is enough to absorb
-- accidental rapid re-scans while still keeping the anti-spam guard.)
-- Keeps the pg_advisory_xact_lock race guard from 20260630030000.
